...
**Image Upload Pipeline**: `ImageUploader` component handles file selection → `compressImage()` → upload to R2 via `@aws-sdk/client-s3` `PutObjectCommand` → store returned URL in `images[]` array. Show upload progress indicator per image.
...
**`EditorControls.tsx`**:
- **Header Config**: Logo upload (uses same `ImageUploader` + R2 pipeline). Updates `stores.logo_url`.
...
3. **Proof Ingestion**: File upload for bank receipt photo (uses `ImageUploader` → R2). CTA button: `إرسال الإشعار وتأكيد الدفع`.
...
- **execution_prompt**: Card allocates **60% vertical space to thumbnail** image (WebP via R2 pipeline, `object-fit: cover`). Below image: product name (single line, ellipsis), price block.
...
6. Build HTML document with:
   ...
   - `<meta property="og:image" content="{product.images[0] via R2 WebP URL}" />`
...
### P4-01 — R2 Bucket Provisioning
- **status**: Todo
- **dependencies**: P0-01
- **repo_path**: `wrangler.toml` (root or per-package)
- **execution_prompt**: Create R2 bucket via Wrangler CLI: `wrangler r2 bucket create vizzo-assets`. Configure CORS policy allowing uploads from `app.vizzotrade.com` origin. Set up R2 API token with read/write permissions. Document env vars: `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET_NAME`, `R2_PUBLIC_URL`. Add to `.env.example` files in dashboard package.

### P4-02 — Cloudflare Image Transformations
- **status**: Todo
- **dependencies**: P4-01
- **repo_path**: Documentation / Cloudflare Dashboard config
- **execution_prompt**: Enable Image Transformations on the R2 bucket's public domain. Configure transformation URL pattern: `{R2_PUBLIC_URL}/{key}?format=webp&quality=80&width=600`. Storefront image `<img>` tags must use this transformation URL (not raw R2 URL) to serve WebP at 80% quality ≈ 40KB. Dashboard can use raw URLs for admin thumbnails. Document the URL pattern in `@vizzo/shared/constants` as `buildImageUrl(key: string, opts?: { width?: number }): string`.
...
### P4-05 — Admin Panel (Subscription Approval)
- **status**: Todo
- **dependencies**: P2-15, P0-04
- **repo_path**: `packages/admin/` (new package, minimal)
- **execution_prompt**: Minimal internal tool (not public-facing). Simple authenticated page (hardcoded admin email check). Displays queue of `subscriptions` where `status = 'pending'`. Each entry shows: store name, merchant email, tier selected, receipt image (from R2), submission date. Two action buttons: **Approve** (updates `status = 'active'`, sets `stores.is_pro = true`, computes `expires_at` based on tier) and **Reject** (updates `status = 'rejected'`). Uses `service_role` Supabase key (server-side only, never exposed to client). Can be a simple Cloudflare Pages app or even a Supabase Dashboard SQL shortcut initially.
...
graph TD
    ...
    subgraph "Phase 4: Infrastructure"
        P4-01["P4-01: R2 Bucket"]
        P4-02["P4-02: Image Transforms"]
        P4-03["P4-03: RLS Hardening"]
        P4-04["P4-04: Discount Cron"]
        P4-05["P4-05: Admin Panel"]
    end
...
| Phase 4 — Infrastructure | 5 | R2, image transforms, RLS, discount cron, admin panel |
| **Total** | **50** | |
