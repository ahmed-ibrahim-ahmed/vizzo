...
| Object Storage | Cloudflare R2 | S3-compatible API via @aws-sdk/client-s3 | Image asset storage |
| Edge Hosting | Cloudflare Pages | Managed service | Static asset delivery & SSR functions |
| CLI / Deploy | Wrangler | 4.x | Cloudflare Pages & R2 management |
| Image Compression (Client) | browser-image-compression | 2.0.x | Pre-upload compression ≤ 200 KB |
| Image Delivery (Server) | Cloudflare Image Transformations (R2 pipeline) | Managed | WebP @ 80% quality, ≈ 40 KB output |
...
4. **Image Pipeline**: Upload path: Client compress (≤ 200 KB) → R2 bucket. Delivery path: R2 → Cloudflare Image Transformations (WebP, 80% quality, ≈ 40 KB) → Buyer browser.
...
│  vizzotrade.com/:slug/p/:id → Product page (SSR via      │
│                               Cloudflare Pages Function)  │
│                                                          │
│  R2 Bucket               → Image assets (WebP pipeline)  │
